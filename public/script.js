...
async function confirmTransfer() {
    ...
    const recipientIdValue = document.getElementById('transferUserId').value.trim();
    ...
    const recipientUserId = parseInt(recipientIdValue, 10);
    ...
    try {
        ...
        if (result.success) {
            ...
            const recipientDisplay = result.recipientUsername ? `@${result.recipientUsername}` : `ID ${result.recipientUserId || recipientUserId}`;

            console.log(`Передача успешна: "${currentTransferItem.name}" отправлен ${recipientDisplay}`);

            // Показываем успешное сообщение с информацией о доставке
            if (window.Telegram?.WebApp?.showPopup) {
                window.Telegram.WebApp.showPopup({
                    title: 'Подарок передан! 🎉',
                    message: `Подарок "${currentTransferItem.name}" успешно передан пользователю ${recipientDisplay}!\n\nПолучатель получит уведомление в боте.`,
                    buttons: [{ type: 'ok', text: 'Отлично!' }]
                });
            } else {
                alert(`Подарок "${currentTransferItem.name}" успешно передан пользователю ${recipientDisplay}!\n\nПолучатель получит уведомление в боте.`);
            }
        ...
        if (error.message.includes('HTTP 404') || error.message.includes('не найден в боте') || error.message.includes('нету в боте')) {
            errorMessage = `Пользователя с таким ID нету в боте.\n\nПолучатель должен:\n• Зайти в бот хотя бы один раз\n• Или быть в списке активных пользователей`;
        } else if (error.message.includes('HTTP 400')) {
            errorMessage = 'Неверные данные для передачи. Проверьте правильность ID.';
...
function subscribeToChannel() {
    const channelUrl = 'https://t.me/WaveGiftNew';
...
// Invite modal functions (removed - referral system disabled)
...
async function openPaymentMethodsModal(item) {
    try {
        ...
        if (window.Telegram?.WebApp?.showPopup) {
            window.Telegram.WebApp.showPopup({
                title: 'Ошибка',
                message: 'Не удалось загрузить способы оплаты. Попробуйте еще раз.',
                buttons: [{ type: 'ok', text: 'OK' }]
            });
        }
    }
...
async function confirmBalancePayment() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            let errorMessage = 'Не удалось завершить покупку с баланса. Попробуйте еще раз.';
            if (error.error === 'Не достаточно средств.') {
...
function getReferrerId() {
    return null; // Referral system disabled
}
